...
@section('title', 'Catálogo de Roles')
...
<div class="space-y-6 w-full px-4 sm:px-6 lg:px-8 mx-auto animate-fade-in text-slate-700 font-medium text-sm">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
        <div>
            <h2 class="text-2xl font-black text-slate-800 tracking-tight uppercase">Catálogo de Roles de Sistema</h2>
            <p class="text-slate-500 text-sm mt-0.5">Gestión de niveles de acceso, permisos y auditoría del personal clínico.</p>
        </div>
    ...
    <div class="bg-slate-800 p-5 rounded-xl shadow-sm border border-slate-700 text-white space-y-4">
        <div class="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-slate-400">
            <i class="fa-solid fa-filter text-blue-400"></i> Filtros de Auditoría
        </div>
        
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <div class="lg:col-span-2">
                <label class="block text-xs font-bold text-slate-300 uppercase mb-1">Buscar Rol (Nombre / Descripción)</label>
                <input type="text" id="filtroTexto" oninput="resetearPaginaYFiltrar()" placeholder="Escribe el rol..." class="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-400 outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            
            <div>
                <label class="block text-xs font-bold text-slate-300 uppercase mb-1">Estado de Acceso</label>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-4 bg-white">
            <table class="w-full text-left border-collapse table-auto border border-slate-200" id="tablaMaestraRoles">
                <thead>
                    <tr class="bg-slate-100 text-slate-600 uppercase tracking-wider border-b border-slate-200 text-xs">
                        ...
                        <th class="p-4 font-bold border border-slate-200 w-[25%]">Rol de Sistema</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700" id="cuerpoTablaRoles">
                    @forelse($roles as $index => $rol)
                    @php
                        $horaRegistro = $rol->created_at ? \Carbon\Carbon::parse($rol->created_at)->subHours(4)->format('d/m/Y H:i') : 'No registrada';
                        $horaInactivacion = $rol->trashed() ? \Carbon\Carbon::parse($rol->deleted_at)->subHours(4)->format('d/m/Y H:i') : '';
                        $estadoFiltro = $rol->trashed() ? 'INACTIVO' : 'ACTIVO';
                        $textoBusqueda = strtolower($rol->nombre . ' ' . $rol->descripcion);
                    @endphp
                    <tr class="hover:bg-slate-50/80 transition-colors {{ $rol->trashed() ? 'bg-slate-50 opacity-60' : '' }}"
                        ...
                        <td class="p-4 font-bold text-blue-700 uppercase border border-slate-100"><i class="fa-solid fa-user-shield mr-2 text-slate-400"></i>{{ $rol->nombre }}</td>
                        <td class="p-4 text-slate-500 border border-slate-100 italic break-words whitespace-normal">{{ $rol->descripcion ?? 'Sin descripción detallada registrada.' }}</td>
                        ...
                        <td class="p-4 font-mono border border-slate-100 text-xs">
                            ...
                            @else
                                <span class="text-slate-400 italic">Vigente / Activo</span>
                            @endif
                        </td>
                        
                        <td class="p-4 text-center border border-slate-100">
                            @if($rol->trashed())
                                <form action="{{ route('roles.restore', $rol->id) }}" method="POST" id="form-restore-{{ $rol->id }}">
                                    @csrf
                                    <label class="relative inline-flex items-center cursor-pointer select-none">
                                        <input type="checkbox" class="sr-only peer" onchange="document.getElementById('form-restore-{{ $rol->id }}').submit()">
    ...
    <div class="flex justify-center gap-1.5">
        ...
        <button onclick='abrirModalVer(@json($rol))' class="bg-slate-500 hover:bg-slate-600 text-white p-2 rounded-lg shadow-sm transition-colors" title="Ver Detalles">
            <i class="fa-solid fa-eye text-xs"></i>
        </button>

        @if(!$rol->trashed())
            <button onclick='abrirModalEditar(@json($rol))' class="bg-[#007bff] hover:bg-blue-700 text-white p-2 rounded-lg shadow-sm transition-colors" title="Editar">
                <i class="fa-solid fa-pen-to-square text-xs"></i>
            </button>
            <button onclick="abrirModalEliminar({{ $rol->id }})" class="bg-red-500 hover:bg-red-600 text-white p-2 rounded-lg shadow-sm transition-colors" title="Inactivar">
                <i class="fa-solid fa-trash text-xs"></i>
            </button>
        @else
            <form action="{{ route('roles.force_delete', $rol->id) }}" method="POST" class="inline-block m-0" onsubmit="return confirm('⚠️ ¡ADVERTENCIA EXTREMA!\n\n¿Estás absolutamente seguro de eliminar este ROL FÍSICAMENTE?\n\nEsta acción NO se puede deshacer.');">
                @csrf
                @method('DELETE')
        ...
        @endif
                    ...
                    <tr id="filaTablaVacia">
                        <td colspan="7" class="p-8 text-center text-slate-400 font-medium">
                            <i class="fa-solid fa-user-lock text-3xl block mb-2 opacity-30"></i> No se encuentran roles de sistema indexados.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<div id="modalVer" class="fixed inset-0 z-50 bg-slate-900/60 hidden flex items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in">
        <div class="flex justify-between items-center p-5 border-b bg-slate-800 text-white">
            <h3 class="text-base font-bold"><i class="fa-solid fa-user-shield mr-2 text-blue-400"></i> Detalles de Rol</h3>
        ...
        <div class="p-6 space-y-4 text-sm text-slate-700">
            <div class="bg-slate-50 p-4 rounded-lg border border-slate-100">
                <p class="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Nombre del Rol</p>
                <p id="ver_nombre" class="text-lg font-black text-blue-700 uppercase"></p>
            </div>
            <div>
                <p class="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Descripción y Permisos</p>
...
<div id="modalRegistro" class="fixed inset-0 z-50 bg-slate-900/60 hidden flex items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100 animate-fade-in">
        <div class="flex justify-between items-center p-5 border-b border-slate-200 bg-slate-50">
            <h3 class="text-base font-bold text-slate-700">Nuevo Rol de Acceso</h3>
        ...
        <form action="{{ route('roles.store') }}" method="POST" autocomplete="off">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-bold text-slate-600 uppercase mb-1">Nombre del Rol</label>
                    <input type="text" name="nombre" placeholder="Ej. Administrador, Médico..." pattern="[a-zA-ZñÑáéíóúÁÉÍÓÚ\s]+" title="Solo se permiten letras" class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none uppercase" required>
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-600 uppercase mb-1">Descripción de Funciones</label>
                    <textarea name="descripcion" rows="3" placeholder="Detalles de los permisos y accesos..." class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none"></textarea>
                </div>
            </div>
...
<div id="modalEditar" class="fixed inset-0 z-50 bg-slate-900/60 hidden flex items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in">
        <div class="flex justify-between items-center p-5 border-b bg-slate-50">
            <h3 class="text-base font-bold text-slate-700">Modificar Rol</h3>
        ...
        <form id="formEditar" method="POST" autocomplete="off">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-bold text-slate-600 mb-1">Nombre del Rol</label>
...
<div id="modalEliminar" class="fixed inset-0 z-50 bg-slate-900/60 hidden flex items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-fade-in text-center">
        <div class="p-6">
            ...
            <h3 class="text-xl font-bold text-slate-800 mb-2">¿Inactivar Rol?</h3>
            <p class="text-sm text-slate-500">Este nivel de acceso no podrá asignarse a nuevos médicos en el sistema.</p>
        </div>
...
<script>
    ...
    function ejecutarFiltrosCombinados() {
        ...
        const filas = document.querySelectorAll('#cuerpoTablaRoles tr');
        let filasFiltradas = [];
    ...
    function exportarExcelNativo() {
        let csv = [];
        let headerData = ["#", "Rol de Sistema", "Descripcion Funcional", "Hora Registro", "Hora Inactivacion"];
        csv.push(headerData.join(";"));

        let cuerpoFilas = document.querySelectorAll('#cuerpoTablaRoles tr');
        ...
        link.download = "Reporte_Roles_Sistema_" + new Date().toISOString().slice(0,10) + ".csv";
    ...
    function imprimirReporteNativo() {
        ...
        ventanaImpresion.document.write('<html><head><title>Reporte - Roles</title>');
        ...
        ventanaImpresion.document.write('<h2>Clínica Vitruvio - Catálogo de Roles de Sistema</h2>');
        ventanaImpresion.document.write('<p>Reporte de auditoría emitido el: ' + new Date().toLocaleString() + '</p>');
        
        ventanaImpresion.document.write('<table><thead>' + document.querySelector('#tablaMaestraRoles thead tr').innerHTML + '</thead><tbody>');
        ...
        ventanaImpresion.document.write('});');
        ventanaImpresion.document.write('<\/script></body></html>');
        ventanaImpresion.document.close();
        setTimeout(function() { ventanaImpresion.print(); ventanaImpresion.close(); }, 500);
    }

    function abrirModalVer(rol) {
        document.getElementById('ver_nombre').innerText = rol.nombre;
        document.getElementById('ver_descripcion').innerText = rol.descripcion || 'Sin descripción funcional registrada.';
    ...
    function abrirModalEditar(rol) {
        ...
        document.getElementById('formEditar').action = "/roles/" + rol.id;
        document.getElementById('modalEditar').classList.remove('hidden');
    }

    function abrirModalEliminar(id) {
        document.getElementById('formEliminar').action = "/roles/" + id;
