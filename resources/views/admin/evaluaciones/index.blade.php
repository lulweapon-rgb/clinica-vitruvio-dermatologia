...
@section('title', 'Historial Clínico Dermatológico')
...
<div class="space-y-6 w-full px-4 sm:px-6 lg:px-8 mx-auto animate-fade-in text-slate-700 font-medium text-sm">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
        <div>
            <h2 class="text-2xl font-black text-slate-800 tracking-tight uppercase">Historial Clínico Dermatológico</h2>
            <p class="text-slate-500 text-sm mt-0.5">Gestión transaccional de consultas, triage pre-análisis y banco de imágenes dermatológicas.</p>
        </div>
    ...
    <div class="bg-slate-800 p-5 rounded-xl shadow-sm border border-slate-700 text-white space-y-4">
        <div class="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-blue-400">
            <i class="fa-solid fa-filter"></i> Filtros de Auditoría para Historiales
        </div>
            ...
            <table class="w-full text-left border-collapse table-auto whitespace-nowrap" id="tablaMaestraEvaluaciones">
                <thead>
                    <tr class="bg-slate-100 text-slate-600 uppercase tracking-wider border-b border-slate-200 text-[10px]">
                        ...
                        <th class="p-3 font-bold border-b border-r border-slate-200">Fecha / Hora</th>
                        <th class="p-3 font-bold border-b border-r border-slate-200">Paciente</th>
                        <th class="p-3 font-bold border-b border-r border-slate-200">Médico Evaluador</th>
                        <th class="p-3 font-bold border-b border-r border-slate-200">Síntomas Clínicos</th>
                        <th class="p-3 font-bold text-center border-b border-r border-slate-200">Muestra Foto</th>
                        <th class="p-3 font-bold text-center border-b border-r border-slate-200 bg-blue-50 text-blue-700">Métrica IA (CNN)</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700 text-xs" id="cuerpoTablaEvaluaciones">
                    @forelse($evaluaciones as $index => $eval)
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors {{ $eval->trashed() ? 'bg-slate-50 opacity-60' : '' }}" 
                            data-busqueda="{{ $textoBusqueda }}" 
                            data-estado="{{ $estadoFiltro }}" 
                            data-medico="{{ $eval->medico_id }}" 
                            ...
                            <td class="p-3 font-bold text-slate-600 uppercase border-r border-slate-100">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[9px] text-blue-600 font-mono"><i class="fa-solid fa-envelope mr-0.5"></i>{{ $eval->medico->usuario->correo ?? 'S/N' }}</span>
                                </div>
                            </td>
                            ...
                            <td class="p-3 text-center">
                                <div class="flex justify-center gap-1">
                                    ...
                                    @else
                                        <form action="{{ route('evaluaciones.force_delete', $eval->id) }}" method="POST" class="inline-block m-0" onsubmit="return confirm('⚠️ ¡ADVERTENCIA EXTREMA!\n\n¿Estás absolutamente seguro de eliminar esta EVALUACIÓN FÍSICAMENTE?\n\nEsta acción borrará la consulta, métricas de IA y fotos permanentemente.');">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <div class="space-y-4 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                    ...
                    <div>
                        ...
                        <input type="file" name="imagen_lesion" accept="image/*" class="w-full border border-blue-200 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-blue-500 outline-none bg-blue-50/50 text-blue-700 font-bold" required>
                    ...
                    <div>
                        ...
                        <textarea name="diagnostico_clinico" rows="4" placeholder="Detalle la morfología observada en la lesión (Asimetría, bordes irregulares, coloración heterogénea)..." class="w-full border rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-blue-500 outline-none resize-none bg-white shadow-sm" required></textarea>
                    </div>
                </div>
                
                <div class="space-y-4 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                    <h4 class="text-xs font-black text-slate-400 uppercase tracking-wider border-b pb-2 flex items-center gap-2">
                        <i class="fa-solid fa-clipboard-check text-emerald-500"></i> 2. Sintomatología de la Lesión
                    </h4>
                    <p class="text-[11px] text-slate-400 leading-normal mb-2">Marcar las condiciones observadas o reportadas por el paciente (Tiempo de Evolución será estimado por IA):</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea name="diagnostico_clinico" id="edit_diagnostico" rows="6" class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none" required></textarea>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden animate-fade-in flex flex-col md:flex-row my-auto max-h-[90vh]">
        <div class="w-full md:w-5/12 bg-slate-900 p-6 flex flex-col justify-center items-center relative">
            ...
            <div class="mt-6 w-full bg-slate-800 border border-slate-700 p-4 rounded-lg text-center">
                ...
                <p id="ver_ia_tiempo" class="text-[9px] text-slate-400 mt-1 uppercase font-mono"></p>
            </div>
        </div>
        <div class="w-full md:w-7/12 p-6 flex flex-col justify-between overflow-y-auto">
            <div>
                ...
                <div class="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-lg border border-slate-100 mb-4">
                    <div><p class="text-[10px] text-slate-400 font-bold uppercase">Fecha y Hora Registro</p><p id="ver_fecha" class="font-bold text-slate-700 text-xs font-mono"></p></div>
...
<script>
    ...
    function abrirModalVer(evaluacion, paciente, medico, urlImagen) {
        ...
        if (evaluacion.ia_porcentaje) {
            ...
            document.getElementById('ver_ia_tiempo').innerText = 'Evolución Estimada: ' + (evaluacion.tiempo_evolucion || 'Desconocida');
        } else {
            ...
            document.getElementById('ver_ia_tiempo').innerText = '';
        }
    ...
    /* EXPORTACIÓN Y REPORTES LIMPIOS */
    function exportarExcelNativo() {
        let csv = [];
        let headerData = ["#", "Fecha/Hora", "Paciente", "Evaluador", "Sintomas", "Diagnostico Clinico", "Estado"];
