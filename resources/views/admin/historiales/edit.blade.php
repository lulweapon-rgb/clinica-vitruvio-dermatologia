...
<div class="space-y-6 w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 animate-fade-in text-slate-700 font-medium text-sm">
    ...
    <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-black text-slate-800 tracking-tight uppercase">Actualizar Expediente Base</h2>
            <p class="text-slate-500 text-sm mt-0.5">Modificación de antecedentes estáticos y factores de riesgo.</p>
        </div>
        <div class="w-12 h-12 bg-[#007bff] text-white rounded-xl flex items-center justify-center text-xl shadow-md border border-blue-600">
    ...
    <form action="{{ route('historiales.update', $paciente->id) }}" method="POST" autocomplete="off" class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        @csrf
        @method('PUT')
        <div class="p-6 space-y-6">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <textarea name="exposicion_sol" rows="2" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none bg-slate-50 shadow-inner">{{ $paciente->antecedentes->exposicion_sol }}</textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-200 shadow-sm cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 transition-all select-none">
                        ...
                        <input type="checkbox" name="quemaduras_previas" {{ $paciente->antecedentes->quemaduras_previas ? 'checked' : '' }} class="w-5 h-5 rounded text-blue-600 border-slate-300 focus:ring-blue-500 shadow-sm cursor-pointer">
                    </label>
                    ...
                    <label class="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-200 shadow-sm cursor-pointer hover:border-red-300 hover:bg-red-50/30 transition-all select-none">
                        ...
                        <input type="checkbox" name="historial_familiar_melanoma" {{ $paciente->antecedentes->historial_familiar_melanoma ? 'checked' : '' }} class="w-5 h-5 rounded text-red-600 border-slate-300 focus:ring-red-500 shadow-sm cursor-pointer">
                    </label>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                    <div>
                        ...
                        <input type="text" name="enfermedades_piel_previas" value="{{ $paciente->antecedentes->enfermedades_piel_previas }}" class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white shadow-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-600 mb-1.5 uppercase">Patologías Sistémicas Familiares</label>
                        <input type="text" name="otras_patologias_familiares" value="{{ $paciente->antecedentes->otras_patologias_familiares }}" class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white shadow-sm">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3 p-5 bg-slate-50 border-t border-slate-200">
            ...
            <button type="submit" class="bg-[#007bff] hover:bg-blue-700 text-white font-bold py-2.5 px-8 rounded-lg text-xs uppercase tracking-wide transition-all shadow-md flex items-center gap-2">
                <i class="fa-solid fa-arrows-rotate"></i> Guardar Cambios
            </button>
        </div>
    </form>
</div>
